...
namespace UGCS.Example
{
    public class AppBootstrapper : BootstrapperBase
    {
		...
		private bool _useUdpSocket=Convert.ToBoolean(System.Configuration.ConfigurationManager.AppSettings["useUdpSocket"]);

		private Guid Gufi = Guid.Parse("7e311fd9-410f-47f6-914d-e07a4b111a44");
		//gufi set to example value
		//gufi="";
		...
		protected override async void Configure()
        {
			...
			try
			{
				...
				//login to uss services
				this.authResult = await UgcsLogin();

				//for all vehicle IDs subscribe to the telemetry
				...
				_connection.ConnectUgcs(Settings.Default.UCSLogin, Settings.Default.UCSPassword, () =>
				{
					...
					telemetryListener.SubscribeTelemtry(authResult);
				...
				//}
				//	else throw new ConnectionException("Login into uss failed");

				if (_isInitialized)
				{
					//Gufi = selectedOperation.Gufi;//alot randomn active gufi here
					SetAnraClientSocket();
					//send messages
				}
				else
		...
		private async Task<AuthResult> UgcsLogin()
		{
			...
			authRequest.Email = "admin@flyanra.com";
			authRequest.Password = "Password1";

			return authResult = await proxy.DoLogin(authRequest);
		...
		//get list of active operations from 
		private async Task<String> SetupOperationLookup()
		{
			return null;
		...
		private IAnraSocketClient SetAnraClientSocket()
		{
			anraSocketClient = null;
			anraSocketClient = _useUdpSocket ? (IAnraSocketClient)new AnraUdpClient() : new WebSocketClient(Gufi);
			anraSocketClient.Init(authResult.Token);
			anraSocketClient.Connect();
			//anraSocketClient.onSocketStatusChange += socketClient_onSocketStatusChange;//subscribing to change in the SocjetToggle status
			return anraSocketClient;
		}
		//private void socketClient_onSocketStatusChange(object sender, EventArgs e)
		//{
		//	//SocketToggle.Checked = anraSocketClient != null && anraSocketClient.IsConnected;//will be either true or false based on if the toggle button is pressed
		//	//anraSocketClient.IsConnected;
		//}

		private void DisconnectAnraClient()
		{
